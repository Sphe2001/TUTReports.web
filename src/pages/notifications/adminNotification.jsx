...
import { showAdminMessageModal } from "../../components/ribbon/adminMessageModal";
...
const AdminNotification = () => {
  ...
  const handleNotificationClick = async (note) => {
    // Always mark as viewed first
    try {
      await markNotificationAsViewed(note.id);
      setNotifications((prev) =>
        prev.map((notif) =>
          notif.id === note.id
            ? { ...notif, read: true, isViewedStatus: true }
            : notif
        )
      );
    } catch (error) {
    ...
    // Now handle actions based on role
    if (userRole === "ADMIN") {
